...
class NKCellAgent:
    ...
    Supports two modes:
    - embedding: Uses high-dimensional text embeddings (1024-dim)
    - feature: Uses Dendritic feature vectors (20-dim) - better for anomaly detection
    """
    ...
    def __init__(self, agent_name: str = "nk_cell_001",
        ...
        Args:
            ...
            mode: "embedding" for dense embeddings, "feature" for Dendritic features
        """
    ...
    def train_on_features(self, normal_data: List[Antigen],
        ...
        Feature-based mode works better than embeddings for negative selection
        because the 20-dimensional space is more tractable.

        Args:
            normal_data: List of normal (non-anomalous) antigens
            feature_vectors: Pre-computed Dendritic feature vectors (20-dim)
        """
    ...
    def _generate_feature_detectors(self):
        ...
        Feature space (20-dim) is much more tractable than embedding space (1024-dim).
        """
        if not self.feature_vectors:
            print(f"[{self.agent_name}] Warning: No feature vectors to generate detectors from")
            return

        self.detector_vectors = []
        detectors_generated = 0
        attempts = 0
        max_attempts = self.num_detectors * 20

        # Compute self statistics for smarter detector generation
        all_features = np.array(self.feature_vectors)
        feature_means = np.mean(all_features, axis=0)
        feature_stds = np.std(all_features, axis=0) + 0.01  # Avoid div by zero

        while detectors_generated < self.num_detectors and attempts < max_attempts:
            attempts += 1

            # Generate detector outside "self" region
            # Use larger perturbation to escape self cluster
            if random.random() < 0.5:
                # Random from feature space bounds with noise
                detector = np.random.rand(self.feature_dim) * 1.5 - 0.25  # Range [-0.25, 1.25]
            else:
                # Perturb from self mean with 2-3 std deviation
                perturbation = np.random.normal(0, 2.0, self.feature_dim) * feature_stds
                detector = feature_means + perturbation
                detector = np.clip(detector, -0.5, 1.5)

            # Check if it matches any self pattern
            matches_self = False
            for self_vec in self.feature_vectors:
                distance = np.linalg.norm(detector - self_vec)
                # Euclidean distance threshold (feature space)
                if distance < self.detection_threshold * 2:  # Adjust for feature scale
                    matches_self = True
                    break

            if not matches_self:
                self.detector_vectors.append(detector)
                detectors_generated += 1

        if detectors_generated < self.num_detectors:
            print(f"[{self.agent_name}] Warning: Only generated {detectors_generated}/{self.num_detectors} feature detectors")

    def detect_with_features(self, antigen: Antigen,
                             feature_vector: List[float]) -> AnomalyResult:
        """
        Detect anomaly using Dendritic feature vector.

        Args:
            antigen: Antigen to test
            feature_vector: Pre-computed Dendritic feature vector (20-dim)
        ...
        # Check distance to self patterns
        min_self_distance = float('inf')
        for self_vec in self.feature_vectors:
            distance = np.linalg.norm(fv - self_vec)
            if distance < min_self_distance:
                min_self_distance = distance

        # Check distance to detectors (non-self)
        min_detector_distance = float('inf')
        detector_matches = []
        for i, det_vec in enumerate(self.detector_vectors):
            distance = np.linalg.norm(fv - det_vec)
            if distance < min_detector_distance:
                min_detector_distance = distance
            if distance < self.detection_threshold * 2:
                detector_matches.append(f"detector_{i}")

        # Decision: anomaly if far from self OR close to detectors
        # Normalize distances to [0,1] range
        max_distance = np.sqrt(self.feature_dim)  # Max possible in unit cube
        self_similarity = 1.0 - (min_self_distance / max_distance)
        detector_similarity = 1.0 - (min_detector_distance / max_distance) if self.detector_vectors else 0.0

        is_anomaly = (
            self_similarity < self.detection_threshold or
            len(detector_matches) > 0
        )

        anomaly_score = max(
            1.0 - self_similarity,
            detector_similarity if detector_matches else 0.0
        )
    ...
    def _generate_feature_explanation(self, is_anomaly: bool,
        ...
        if is_anomaly:
            reasons = []
            if self_sim < self.detection_threshold:
                reasons.append(f"low feature similarity to self ({self_sim:.3f})")
            if detector_matches:
                reasons.append(f"matched {len(detector_matches)} non-self detectors")
            return f"Anomaly detected (feature-based): {', '.join(reasons)}"
    ...
    def save_state(self, path: str) -> None:
        ...
        state = {
            ...
            # Feature-based mode data
            "feature_vectors": [fv.tolist() for fv in self.feature_vectors],
            "detector_vectors": [dv.tolist() for dv in self.detector_vectors],
    ...
    def load_state(cls, path: str) -> "NKCellAgent":
        ...
        agent.detector_vectors = [np.array(dv) for dv in data.get("detector_vectors", [])]
        agent.feature_dim = data.get("feature_dim", 20)
        return agent
